...
{ // Model : Head
	...
	{ // BLOCK:set_var:2012082201:입력값 체크 & 초기값 설정
		...
		// #TODO title이 한글일 경우 uid 값을 slug로 이용해야 함
		$arg['slug']     = (!empty($_POST['slug']))          ? $_POST['slug']          : $arg['title'];

        $arg['content']  = (!empty($_POST['post_content']))  ? $_POST['post_content']  : '';
		$arg['rdf_about']= (!empty($_POST['rdf_about']))     ? $_POST['rdf_about']     : '';
        $arg['meta']     = (!empty($_POST['meta']))  ? $_POST['meta'] : array();

		// #TODO file_type 선택
		$arg['data_type'] = $_POST['data_type'];
