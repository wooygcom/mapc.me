...
{ // Model : Head
    ...
    { // BLOCK:original_file:2012080901:원본파일 업로드
        ...
        // 파일 업로드일 경우
            ...
            include(LIBRARY_PATH . 'mapc/file_upload.php');

            $options['request_method'] = $_SERVER['REQUEST_METHOD'];
            mapc_file_upload($_FILES, $options);
            ...
            foreach($_FILES['post_file']['name'] as $key => $var) {
                ...
                { // BLOCK:make_thum:20131122:thumnail 만들기
                    ...
                    $arg['meta']['dc_date'][0]   = $tmp2[0] . '-' . $tmp2[1] . '-' . $tmp2[2] . ' ' . $tmp1[1];
                    $arg['date'] = empty(trim(str_replace('-', '', $arg['meta']['dc_date'][0]))) ? date('Y-m-d H:i:s') : $arg['meta']['dc_date'][0];
                    unset($tmp1, $tmp2);
