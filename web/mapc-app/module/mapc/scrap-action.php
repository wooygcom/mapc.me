...
//        include_once(__DIR__ . '/model/dc_make.func.php');

        include_once(__DIR__ . '/model/thumMake.func.php');
    ...
    { // BLOCK:get_info_of_each_file:20160102

        include_once(LIBRARY_PATH . 'mapc/dir_list.func.php');
        $dirList = mapc_dir_list($dataPathAbs . 'original/', [
            'show_sub' => true,
            'show_base'=> true,
            'ext_exp'  => 'rdf'
        ]);

        foreach($dirList as $each_file) {
                ...
                switch($scrap_type) {
                    case 'thum':
                        ...
                        { // BLOCK:make_thum:20131122:thumnail 만들기
                            ...
                            $fileInfo = mapc_pathinfo_utf($each_file);

                            if(in_array(strtolower($fileInfo['extension']), ['jpeg', 'jpg', 'gif', 'png'])) {

                                // 썸네일 디렉토리가 없을 경우 생성
                                $thumDir = str_replace('original/', 'thum/', $fileInfo['dirname']);
                                if(! is_dir($thumDir)) {
                                    $tmp_dir_name = (PHP_OS == 'WINNT') ? str_replace("/", "\\", $thumDir) : $thumDir;
                                    mkdir($tmp_dir_name, 0755, true);
                                    unset($tmp_dir_name);
                                }

                                // 썸네일 화일이 없으면 생성하기
                                if(! is_file($thumDir . '/' . $fileInfo['basename'])) {
                                    module_mapc_thum_make(
                                        ...
                                        [
                                            'min' => 768,
                                            'max' => 1024,
                                            'copyright' => $dataPathAbs . 'custom/copyright.png'
                                        ]
                                    );
                                }
                            }
...
// /////////////////////////////////////////////////////////////////////////////////////////////////////* /
                    ...
                    if(is_file($each_file . '.rdf')) {
                        ...
                        $dc_info->mapc_dir  = str_replace(array($dataPathAbs.'original/', $file_info['basename']), '', $each_file);
                        ...
                        switch($mime_type[0]) {
                            ...
                            case 'image':
                                { // BLOCK:make_thum:20131122:thumnail 만들기
                                    ...
                                    // 썸네일 디렉토리가 없을 경우 생성
                                    $thumDir = $dataPathAbs . 'thum/' . $dc_info->mapc_dir;
                                    if(! is_dir($thumDir)) {
                                        $tmp_dir_name = (PHP_OS == 'WINNT') ? str_replace("/", "\\", $thumDir) : $thumDir;
                                        mkdir($tmp_dir_name, 0755, true);
                                        unset($tmp_dir_name);
                                    ...
                                    // 썸네일 화일이 없으면 생성하기
                                    if($scrap_option_thum == 'thum' && ! is_file($dataPathAbs . 'thum/' . $dc_info->mapc_dir . $file_info['basename'])) {
                                        module_mapc_thum_make(
                                            $dataPathAbs . 'original/' . $dc_info->mapc_dir,
                                            $dataPathAbs . 'thum/' . $dc_info->mapc_dir,
                                            $file_info['basename'],
                                            $mime_type[1],
                                            $option
                                        );
                    ...
                    // rdf화일이 없을 경우 rdf화일을 새로 만듦
                        ...
                        $tmp_subjects = explode("/", dirname( str_replace($dataPathAbs."original/", "", $each_file) ) );
                        ...
                        { // BLOCK:value_set_dc:20131217:더블린 코어에 들어갈 값들 지정
                            ...
                            $dataPathAbs_dc   = $file_info['dirname'] . '/';
                        ...
                        { // BLOCK:dc_file_make:2012080901:메타데이타(더블린코어) 파일 생성
                            ...
                            if($scrap_option_rdf == 'rdf') {
                                ...
                                module_mapc_dc_make($file_info['basename'] . '.rdf', $arg['meta'], $dataPathAbs_dc);
