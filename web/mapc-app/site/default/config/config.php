...
{ // BLOCK:basic_config:20121202:기본 환경설정
    ...
    $CONFIG['lang']['support']= ['ko-KR', 'en-US'];
...
{ // BLOCK:meta:20141219:메타데이터
    ...
    // 사이트 제목 지정
    $VIEW['meta']['title']     = _('사이트 제목');
    // 저작권자 지정
    $VIEW['meta']['copyright'] = _('[YOURSITEDOMAIN].com');
    // 키워드 지정
    $VIEW['meta']['keywords']  = _('MAPC, 사이트 키워드1, 사이트 키워드2');
    // 사이트 주제(문장)
    $VIEW['meta']['subject']   = _('사이트 주제1');
    // 사이트 설명
    $VIEW['meta']['description'] = _('사이트 설명');
    // 권한자
    $VIEW['meta']['author']    = _('권한자');
    // 작성자
    $VIEW['meta']['writer']    = _('글쓴이');
    // 웹로봇 설정
    $VIEW['meta']['robots']    = 'all';
    // 기본 언어
    $VIEW['meta']['content-language'] = 'ko';
...
{ // BLOCK:secret_config:20130104:템플릿으로 출력되기 직전 삭제할 환경설정
    ...
    $CONFIG_SECRET['session']['save_session_path'] = TEMP_PATH . 'session/';
...
{ // BLOCK:path_config:20121202:.....
    ...
    // 사이트의 VIEW화일이 들어있는 기본 디렉토리
    $PATH['view']['site']   = VIEWS_PATH;
    // 각 ADMIN의 VIEW화일이 들어있는 기본 디렉토리 (넘어오는 modl에 따라 변경됨)
    $PATH['view']['admin']  = ADMIN_PATH . $ARGS['admn'] . '/view/' . $CONFIG['skin'] . '/';
    // 각 모듈의 VIEW화일이 들어있는 기본 디렉토리 (넘어오는 modl에 따라 변경됨)
    $PATH['view']['module'] = MODULE_PATH . $ARGS['modl'] . '/view/' . $CONFIG['skin'] . '/';

    $URL = array();
    // ROOT
    $URL['core']['root']  = pathinfo($_SERVER['SCRIPT_NAME'], PATHINFO_DIRNAME);
    // Add '/'(Slash) if not exist
    $URL['core']['root'] .= substr($URL['core']['root'], -1) == '/' ? '' : '/';
    $URL['core']['admin'] = $URL['core']['root'] . 'admin-';
    $URL['core']['admin_main'] = $CONFIG['root_url'] . 'admin-core/';
    $URL['core']['abs_root'] = 'http' .
        ((isset($_SERVER['HTTPS']) && $_SERVER['HTTPS']=='on') ? 's' : '') . '://' .
        $_SERVER['HTTP_HOST'] . '/' . $URL['core']['root'];
