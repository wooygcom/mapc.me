    ...
    <article class="blog-post">
        ...
        <div class="panel panel-info">
            <div class="panel-heading">
                <h1 class="panel-title"><?= $post['main']['post_title']; ?></h1>
            </div>
            <div class="panel-body">
    
                <?php
                    $sep = '';
                    if(! empty($post['main']['post_write_date'])) {
                        echo $sep . '글쓴날 : ' . $post['main']['post_write_date'];
                        $sep = ' / ';
                    }
                    if(! empty($post['main']['post_edit_date_latest'])) {
                        echo $sep . '고친날 : ' . $post['main']['post_edit_date_latest'];
                        $sep = ' / ';
                    }
                ?>
        ...
        <!-- 연결 : H -->
        <p>
            [ <a href="<?= $URL['core']['root']; ?>mapc/<?= $post['main']['post_uid']; ?>/edit/&amp;mapc-lang=<?= $post['main']['post_lang']; ?>">편집</a>
            / <a href="<?= $URL['core']['root']; ?>mapc/<?= $post['main']['post_uid']; ?>/edit/&amp;mapc-lang=<?= $post['main']['post_lang']; ?>">삭제</a> ]
            [
            <?php
                $sep = '';
                if($view_prev[0]) {
            ?>
            <a href="<?= $URL['mapc']['view']; ?>mapc_uid/<?= $view_prev[0]; ?>/mapc_lang/<?= $post['main']['post_lang']; ?>">이전글</a>
            <?php
                    $sep = '/';
                }
                if($view_next[0]) {
                    echo $sep;
            ?>
            <a href="<?= $URL['mapc']['view']; ?>mapc_uid/<?= $view_next[0]; ?>/mapc_lang/<?= $post['main']['post_lang']; ?>">다음글</a>
            <?php
                }
            ?>
            ]
        ...
        <div class="panel panel-info">
            ...
            <div class="panel-body">
                ...
                ?>
                    ...
                    <?php
                        ...
                        foreach($title_another_lang as $var) {
                            echo $temp_sep . ' <a href="' . $URL['mapc']['view'] . 'mapc_uid/' . $post['main']['post_uid'] . '/mapc_lang/' . $var['postmeta_lang'] . '">'
                ...
                ?>
                    ...
                    <?php
                        ...
                        foreach($title_same_subject as $var) {
                            echo $temp_sep . ' <a href="' . $URL['core']['root'] . 'mapc/' . $var['post_uid'] . '/edit/&mapc_lang=' . $var['post_lang'] . '">'
                                . $var['post_title']
                                . '</a>';
                            $temp_sep = ', ';
                        }
                ...
                ?>
                        ...
                        <p>
                            제목 : [<?= $post['main']['post_title']; ?>]으로 검색하기 :
                            <a href="http://www.google.com/search?q=<?= $post['main']['post_title']; ?>">구글</a> / 
                            <a href="http://search.naver.com/search.naver?query=<?= $post['main']['post_title']; ?>">네이버</a> / 
                            <a href="http://search.daum.net/search?q=<?= $post['main']['post_title']; ?>">다음</a> / 
                            <a href="http://ko.wikipedia.org/wiki/<?= $post['main']['post_title']; ?>">위키피디아</a>
                        </p>
...
<script>
    <?php
        $size_info = getimagesize($PATH['mapc']['data'].$post['main']['post_origin_url']);
    ?>
    var openPhotoSwipe = function () {
        var pswpElement = document.querySelectorAll('.pswp')[0];
        var items = [
            {
                src: '<?= $URL['mapc']['file_view'] . 'mapc_uid/' . $post['main']['post_uid']; ?>',
                w:<?= $size_info[0]; ?>,
                h:<?= $size_info[1]; ?>
            }
        ];
